  ...
  return (
    <div className='py-10'id='experiance'>
      <h2 className={`w-full text-center text-5xl capitalize md:text-4xl ${inconsolata.className} `}>
        My  <span className={`text-orange-400 text-6xl ${eb.className} `}>Technical Skills</span>
      </h2>
      <div className='flex w-full mt-12 flex-wrap gap-10 justify-center  '>
        {MyEx.map(({title,id,dec1,dec2,icons})=>(
          ...
          <div className='flex flex-wrap lg:flex-row flex-col lg:items-center p-3 py-6 md:p-5 lg:p-10'>
            <h2 className={`text-orange-400 text-lg md:text-2xl m-5 font-bold ${eb.className} `} >{title}</h2>
            <h3 className={`dark:text-yellow-100 text-black text-sm md:text-xl py-2 ${inconsolata.className} `}>{dec1}</h3>
            <h3 className={`dark:text-yellow-100 text-black text-sm md:text-xl py-2 ${inconsolata.className} `}>{dec2}</h3>
